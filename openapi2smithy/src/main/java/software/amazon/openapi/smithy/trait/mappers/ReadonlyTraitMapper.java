...
public class ReadonlyTraitMapper implements OpenApi2SmithyTraitMapper {
    ...
    public Shape applyTrait(Shape shape, Object object, Context context) {
        return Optional.ofNullable(object).filter(HttpTraitMapperContext.class::isInstance)
                ...
                .map(this::getReadonlyTrait)
                .map(trait -> applyTraitToShape(shape, trait))
                .orElse(shape);
    }

    private Trait getReadonlyTrait(String operationType) {
        if (operationType.equals(GET.toString())) {
            return new ReadonlyTrait();
        }
        return null;
    }
