...
public final class HttpLabelTraitMapper implements OpenApi2SmithyTraitMapper {
    ...
    public Shape applyTrait(Shape shape, Object object, Context context) {
        return Optional.ofNullable(object).filter(Parameter.class::isInstance).map(Parameter.class::cast)
                ...
                .filter(in -> in.equals("path"))
                .map(path -> new HttpLabelTrait()) // create the trait
                .map(trait -> applyTraitToShape(shape, trait))
                //HttpLabel trait should always be accompanied by RequiredTrait
                .map(shapeObj -> applyTraitToShape(shapeObj, new RequiredTrait()))
                .orElse(shape);
    }
}
