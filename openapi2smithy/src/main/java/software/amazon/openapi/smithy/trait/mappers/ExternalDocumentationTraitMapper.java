...
public final class ExternalDocumentationTraitMapper implements OpenApi2SmithyTraitMapper {
    ...
    public Shape applyTrait(Shape shape, Object object, Context context) {
        return Optional.ofNullable(object).filter(OpenAPI.class::isInstance).map(OpenAPI.class::cast)
                .map(OpenAPI::getExternalDocs)
                .map(ExternalDocumentation::getUrl)
                .filter(item -> !item.isEmpty())
                .map(ExternalDocumentationTrait::new)
